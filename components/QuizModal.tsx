...
interface QuizModalProps { lessonTitle: string; lessonId: string; onClose: () => void; onXPEarned: (xp: number) => void; }
...
export default function QuizModal({ lessonTitle, onClose, onXPEarned }: QuizModalProps) {
  ...
  useState(() => {
    fetch("/api/quiz", {
      ...
      .catch(() => { setLoading(false); toast.error("Could not load quiz"); onClose(); });
  });
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          {loading ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-5xl font-black text-gradient mb-4">{score}/{questions.length}</p>
            ...
            <>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-1">{questions.map((_, i) => (
                  <div key={i} className={`w-2 h-2 rounded-full ${i < current ? "bg-green-500" : i === current ? "bg-blue-500" : "bg-slate-700"}`} />
                ))}</div>
              </div>
              <h3 className="text-xl font-semibold text-white mb-6">{questions[current].question}</h3>
              <div className="space-y-3">
                {questions[current].options.map((opt, i) => {
                  let cls = "glass border border-white/10 text-slate-200 hover:bg-white/10";
                  if (selected !== null) {
                    if (i === questions[current].correct) cls = "bg-green-500/20 border-green-500 text-green-300";
                  ...
                  return (
                    <button key={i} onClick={() => handleAnswer(i)}
                      ...
                      {selected !== null && i === questions[current].correct && <CheckCircle className="w-4 h-4 shrink-0" />}
                      {selected !== null && i === selected && i !== questions[current].correct && <XCircle className="w-4 h-4 shrink-0" />}
                      <span>{opt}</span>
                    </button>
                  );
                })}
